...
export function PatientCompleteness({ completeness }: { completeness: DataCompleteness }) {
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-foreground dark:text-white flex items-center gap-2">
          <span>Data Completeness</span>
          <span className="text-xs font-normal text-muted-foreground">(Registry Standard)</span>
        </h3>
        <span className="text-sm font-bold text-foreground dark:text-slate-100">{completeness.score}%</span>
      </div>

      {/* Progress Bar */}
      <div className="w-full bg-muted rounded-full h-2 dark:bg-slate-800 overflow-hidden mb-4">
        <div
  ...
  return (
    <div className={cn('space-y-1 group relative', className)}>
      <div className="flex items-center justify-between">
        <dt className="text-xs font-medium text-muted-foreground dark:text-slate-400">{label}</dt>
        {source && (
          <span
            className="text-[10px] text-muted-foreground hover:text-primary transition-colors font-medium cursor-help"
      ...
      <dd className="flex items-center gap-2 text-sm font-semibold text-foreground dark:text-slate-100">
...
export function PatientDataSection({
  ...
  return (
    <div className={cn('rounded-xl border border-border bg-card p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900', className)}>
      <div className="flex items-center justify-between border-b border-border pb-4 mb-4 dark:border-slate-800">
        <h2 className="text-base font-bold text-foreground dark:text-slate-100">{title}</h2>
        {action}
      </div>
      {children}
    </div>
