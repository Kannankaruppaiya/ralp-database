-- Production apply bundle for 0012_surgeon_roster.sql
--
-- Run this INSTEAD of psql-ing the migration directly. It is one transaction:
-- the pre-flight abort, the migration, the verification and the history row
-- either all land or none do, so a failure leaves production exactly as it was
-- rather than half-migrated with its views and RLS policies dropped.
--
--   docker exec -i supabase-db psql -U postgres -d postgres -v ON_ERROR_STOP=1 --     < supabase/snippets/apply-0012-production.sql
--
-- (substitute the real container name from `docker ps` if it differs)

\set ON_ERROR_STOP on
begin;

-- ---------------------------------------------------------------- pre-flight
-- The new check constraints are stricter than nothing, so any code already in
-- production that does not match would abort the ALTER halfway. Today every
-- value comes from the enum and matches, but assert it rather than assume it:
-- this is the one thing that differs between production and the local database
-- this migration was tested on.
do $$
declare bad text;
begin
  select string_agg(distinct c, ', ') into bad from (
  ...
  ) v where c !~ '^[A-Z][A-Z0-9]{1,7}$';

  if bad is not null then
    raise exception 'ABORT: surgeon codes already in production do not fit the new format: %', bad;
  end if;

  raise notice 'pre-flight OK: every existing surgeon code fits the new format';
end $$;
...
declare n int;
begin
  if exists (select 1 from pg_type where typname = 'surgeon_code') then
    raise exception 'VERIFY FAILED: the surgeon_code enum is still present';
  end if;

  -- Assert the function runs, not that the registry is non-empty: a legitimately
  -- empty roster is not a failed migration.
  select count(*) into n from surgeon_roster();
  raise notice 'roster returns % codes', n;

  select count(*) into n from pg_policies
   where tablename = 'patients'
     and policyname in ('clinicians read own patients', 'clinicians write own patients');
  if n <> 2 then raise exception 'VERIFY FAILED: caseload scoping policies not restored (found %)', n; end if;
  ...
  if n <> 3 then raise exception 'VERIFY FAILED: only % of 3 views restored', n; end if;

  select count(*) into n from pg_constraint
   where conname in ('profiles_surgeon_code_format', 'patients_primary_surgeon_format', 'operations_surgeon_format');
  if n <> 3 then raise exception 'VERIFY FAILED: only % of 3 format constraints created', n; end if;

  raise notice 'verification OK: enum dropped, roster live, 2 policies + 3 views + 3 constraints in place';
end $$;

-- Keep the migration history straight, if this deployment tracks one.
do $$
begin
  if to_regclass('supabase_migrations.schema_migrations') is not null then
    insert into supabase_migrations.schema_migrations (version, name)
    values ('0012', 'surgeon_roster')
    on conflict (version) do nothing;
    raise notice 'recorded 0012 in schema_migrations';
  end if;
end $$;
